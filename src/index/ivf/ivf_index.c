...
static void kmeans_init_random(const float *vecs, uint32_t n,
    ...
    for (uint32_t i = 0; i < k; i++)
        memcpy(centroids + (size_t)i * dim,
               vecs + (size_t)indices[i] * dim,
               dim * sizeof(float));
...
int ivf_index_train(IvfIndex *idx, const float *train_vecs, uint32_t n) {
    ...
    if (kmeans_train(train_vecs, n, idx->centroids, idx->nlist,
                     idx->dim, dist_fn) < 0) {
        free(idx->centroids);
        idx->centroids = NULL;
        return -1;
    ...
    for (uint32_t i = 0; i < idx->nlist; i++) {
        if (flat_index_create(&idx->partitions[i], idx->bp,
                              idx->dim, idx->metric) < 0) {
...
int ivf_index_search(IvfIndex *idx, const float *query, uint32_t k,
    ...
    for (uint32_t c = 0; c < idx->nlist; c++) {
        float d = dist_fn(query, idx->centroids + (size_t)c * idx->dim,
                          idx->dim);
    ...
    for (uint32_t i = 0; i < centroid_tk.count; i++) {
        ...
        uint32_t part_num = 0;
        int rc = flat_index_search(part, query, part_k,
                                   part_results, &part_num);
        if (rc < 0) {
            free(part_results);
            vec_topk_destroy(&centroid_tk);
            vec_topk_destroy(&final_tk);
        ...
        for (uint32_t j = 0; j < part_num; j++) {
            vec_topk_push(&final_tk,
                          part_results[j].page_id,
                          part_results[j].slot_index,
                          part_results[j].distance);
        }
