...
FLAT_INDEX_SRC = src/index/flat_index.c
...
$(BUILDDIR)/flat_index.o: $(FLAT_INDEX_SRC) src/index/flat_index.h src/index/vec_page.h src/index/vec_utils.h src/index/vec_topk.h src/buffer/buffer_pool.h
	@mkdir -p $(BUILDDIR)
	$(CC) $(CFLAGS) -c $< -o $@
...
test: $(BUILDDIR)/test_pages $(BUILDDIR)/test_storage $(BUILDDIR)/test_storage_bulk $(BUILDDIR)/test_buffer_pool $(BUILDDIR)/test_flat_index
	@echo ""
	@$(BUILDDIR)/test_pages
